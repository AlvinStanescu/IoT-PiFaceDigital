...
{
    /// <summary>
    /// An MCP23S17 negated register nibble.
    /// </summary>
    public class MCP23S17RegisterNibbleNeg : MCP23S17RegisterBaseNeg
    {
        public MCP23S17RegisterBitNeg[] Bits { get; protected set; }

        private readonly Nibble _nibble;

        internal MCP23S17RegisterNibbleNeg(Nibble nibble, byte address, MCP23S17 chip) : base(address, chip)
        {
            this._nibble = nibble;
            this.Bits = new MCP23S17RegisterBitNeg[4];
            ...
            {
                this.Bits[i - rangeStart] = new MCP23S17RegisterBitNeg(i, address, chip);
            }
        }
        ...
        public override byte Value
        {
            get
            {
                ...
                return (byte)(0xF ^ (readByte >> shiftBackValue));
            }
            set
            {
                var registerValue = this._chip.Read(this.Address);
                this._nibble.SetByteNibbleNeg(value, ref registerValue);
